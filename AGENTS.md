...
- `docs/reference/restore-character-spec.md` — normative pre-Level-Up snapshot + Restore Character contract (**phases R1 capture, R2 restore engine, R3 restore-only Restore Character UI, R4 backup asset completeness, R5-A sheet-side base ability scores, R5-B1 required choices + Complete Choices, and R5-B2 under-cap spell choices + persisted acknowledgement are implemented** (2026-07-18 → 2026-08-02); R5-C and R6 still require explicit owner authorization, and **snapshot deletion is deferred to a separately authorized future phase — not R3, not R4/R5/R6**)
- `docs/operations/**`
...
30. [x] **Prepared Correctness C2-D — the Spells panel stops offering a manual `Prepared` override on granted spells, and names each grant for what it is: shipped 2026-07-30 (owner-authorized, plus a same-day review correction).** Two production files (`js/pages/character/panels/spellsPanel.js`, `styles.css`). The sheet was the last surface contradicting the prepared model: a `builderGranted` row carried the same interactive `Prepared` toggle as any other row, titled "Manual/DM prepared override" — implying a grant is an ordinary preparation the player chose, may unchoose, and that it occupies one of the `effectiveCapacity` slots C1 reserves for ordinary candidates. None of that is true (grants reach the sheet through `derived.grantedSpells`, are excluded from `ordinaryCandidateIds`, consume no capacity, and are never eligible for C1.1's projection), so the toggle also had no authoritative meaning to write back to. **Every** row with `builderGranted === true` now renders no `Prepared` control and instead carries a non-interactive marker plus a **visible** sentence about preparation capacity — rendered text on its own wrapping line, not a `title` tooltip and not behind the notes disclosure, so a collapsed granted row still shows it; dropping the third toggle also gives the row's name input *more* width at 380px rather than less. **Marker presence and `Prepared` suppression are decided by `builderGranted` alone** and never consult the stored `prepared` boolean, and render **repairs nothing**: no row is normalized, no persisted field is touched to support the display, and rendering marks nothing dirty. **Only the wording is grant-kind-sensitive, live-derived from the row's `builderSpellId` through the active registry at render time** — finite `level === 0` → `Granted Cantrip` ("Cantrips are not prepared and do not use ordinary prepared spell capacity."), finite `level > 0` → `Always Prepared` (the wizard's own heading, "It stays prepared…"), and a missing/deleted/invalid record → the neutral `Granted Spell` fallback, with an absent or malformed level **never coerced to `0`**. **No `grantType` or any other field was persisted to achieve this.** `Known` and `Cast` keep their existing `mutateSpellEntry()` path, one dirty mark per user action, and full keyboard operation; manual and ordinary builder-managed rows are byte-identical. Both surfaces get it through the one shared `initSpellsPanel()` — the Combat embedded panel is not a second renderer. Tests: `tests/spellsPanel.grantedPresentation.test.js` (14, every negative case with a positive control) and `tests/smoke/grantedSpellPresentation.smoke.js` (3 — a real Life Domain cleric for leveled grants, and a real **High Elf** Life Domain cleric whose required wizard-cantrip choice puts a genuine granted cantrip and granted leveled spells on one sheet; both Spells surfaces, Combat→Character synchronization, reload, 380px). Both files were run against reverted production code first: against `cf446df`, 6 of 9 unit cases and both smoke cases failed; against the pre-correction `ac37895`, 5 of 14 unit cases and the 2 cantrip-covering smoke cases failed while the leveled-grant case passed on both sides as the positive control. Verified: typecheck clean, 1586/1586 unit, build clean, both smoke gates 79/79, plus a real 380 × 820 production-preview pass on the current `dist/` bundle with the service worker unregistered and caches cleared (zero console/page errors, zero page or panel horizontal overflow). **Presentation only: no prepared picker, no Long Rest mutation, no new prepared state, no schema change, no migration, no persisted field, no acknowledgement, no load-time cleanup.** `rest.preparedByClass` stays authoritative and the shared-spell provenance limitation is unchanged. **Not in C2-D:** R5-B2, the cantrip/known/spellbook under-cap acknowledgement, the level-20 Wizard correction path, R5-C, R6, snapshot deletion, per-row spell provenance, the accessibility pass, the dead `getPreparedSpellCapacity()` accessor, and the duplicate Long Rest `deriveCharacter()` call.
31. [x] **R5-B2 — under-cap spell choices become fillable, capped, and acknowledged: shipped 2026-08-02 (owner-authorized).** Six production files. R5-B1 classified class cantrips, known spells, and the wizard spellbook as *permitted* under-cap categories but left three defects: the creation picker's generic handler enforced **no cap at all** (a level-1 wizard could tick every cantrip on the list while the Summary still reported "0 of 3 chosen"), the wizard spellbook was the one category with **no visible maximum**, and Level Up only ever offered the count the appended level *added*, so a caster who came in under at an earlier level had no route back except Edit in Builder — which R5-C retires. `js/domain/rules/choiceCompletion.js` stays the single traversal and count owner and gains the thin `getUnderCapChoiceDescriptors()` accessor plus the acknowledgement model (`normalizeUnderCapAckLevels` / `hasUnderCapAckLevel` / `appendUnderCapAckLevel` / `getResultingCharacterLevel`), so **both wizards cap against exactly the `allowed` the Summary counts against**. Creation now disables unchosen rows at the allowance while chosen ones stay removable, defends the cap inside the change handler against synthetic events, and shows the spellbook's `n / allowed`. Level Up evaluates the **resulting** build at the **resulting total character level**: pickers are capped at the resulting allowance with earlier picks locked as context, a class with no delta still gets a section when it carries a shortfall, and the Spells step becomes available for a shortfall alone. Finishing short stays legal but takes **one** explicit inline acknowledgement (creation's evaluates the C2-B prepared underfill and the under-cap shortfall together, so one extra click covers both; Level Up relabels Apply → `Apply Anyway`), focus moves to the `role="alert"`, a keyboard-operable **Review spell choices** returns to the Spells step, and changing any relevant selection or allowance invalidates the transient confirmation. **State owner: the optional `underCapAckLevels?: number[]` on the character entry** (sibling to `build`/`rest` — a decision *about* a build, not a build choice), aggregated by resulting total level, append-only, deduplicated, ascending, written only inside the successful creation mutation or the single Level Up mutation that also appends the pre-Level-Up snapshot (one dirty mark, one rerender, snapshot transaction intact). Malformed, duplicate, non-integer, negative, and out-of-range values fail soft; nothing is inferred from a shortfall; prepared underfill is never mixed in. Edit in Builder is excluded from the acknowledgement (its draft is not a creation) but **is** subject to the cap fix. **No schema version bump and no migration** — a missing field reads as "nothing acknowledged" — and the field rides through sanitize, the campaign vault, full backup, `.ll-character.json` portability, and snapshot/restore because those carry character entries whole. Tests: new `tests/underCapChoices.test.js` (14), `tests/underCapCreation.test.js` (8), `tests/underCapLevelUp.test.js` (10), `tests/smoke/underCapChoices.smoke.js` (2), plus preservation cases in `tests/state.sanitize.test.js`, `tests/storage.backup.test.js`, `tests/characterPortability.test.js`, `tests/characterSnapshots.restore.test.js` and 10 real-wizard/real-Level-Up cases in `tests/characterPage.test.js`; every negative case carries a positive control, and the three new unit files were run against reverted production code first. **Not in R5-B2:** the level-20 Wizard correction path, R5-C, R6, prepared-spell changes, required-choice banner / Complete Choices behavior, snapshot deletion, and per-row spell provenance.
32. [x] **Max-level spellbook correction — "Add Spellbook Choices" on the Character Spells surface: shipped 2026-08-03 (owner-authorized).** Six production files. R5-B2 made an unused under-cap allowance fillable at the *next* Level Up, but a character already at `MAX_CHARACTER_LEVEL` has no next Level Up: `characterPage.js` disables the menu action, `levelUpWizard.open()` refuses, and `getLevelUpPlan()` returns `null`, while Complete Choices renders only *required* choices by construction — so a stranded spellbook allowance was reachable solely through Edit in Builder, which R5-C retires. The Character page's Spells panel header now carries a `⋯` **overflow menu** (the Abilities & Skills pattern, registered through the shared `Popovers` system with a fallback toggle when it is absent) holding **Add spell level** — the same `#addSpellLevelBtn` element and the same single handler, relocated with no behavior change — and **Add Spellbook Choices**. The new `js/pages/character/spellbookChoicesFlow.js` owns the focused dialog; `getSpellbookCorrectionTargets()` is its pure eligibility/target resolver. **Eligibility is exactly three live conditions** — builder character, total level at `MAX_CHARACTER_LEVEL`, at least one unsatisfied `UNDER_CAP_KIND.SPELLBOOK` descriptor — driven by registry-backed descriptors rather than a hard-coded `"wizard"`, so compatible custom `spellbook` classes are covered and **every** eligible class gets its own section rather than the first one silently winning. **The candidate ceiling is each class's own `maxSpellLevel` from `getPreparedSpellPlan()`, never the combined multiclass slot array** (owner correction to the design proposal): a Wizard 1 / Cleric 19 has 9th-level combined slots and may still add only 1st-level wizard spells. Options resolve through the existing `resolveSpellChoiceOptions(registry, { source: "spells", filter: { classId, minLevel: 1, maxLevel } })`, so the module owns no allowance, ceiling, or candidate formula; an unresolvable ceiling or class record fails **visibly** with no picker and no mutation. The dialog shows the shared descriptor's `chosen / allowed`, renders stored entries as read-only locked context, offers only not-already-stored candidates, disables unchosen rows at the allowance while dialog picks stay removable, and defends the cap inside the change handler against synthetic events. Apply re-resolves character + campaign identity, rechecks level eligibility, recomputes allowance/ceiling/candidates from the live character and registry, rejects any id outside the fresh candidate set or any result above the fresh allowance, then **appends only** to `build.spellcasting[classId].knownIds` (existing ids byte-identical and in order) and assigns the new **spells-only** `getSpellbookAdditionSheetPatch()` — one `mutateCharacter`, one dirty mark, one rerender. That accessor is a sibling of `getLongRestPreparedSheetPatch()` for the same C1.1 reason: the full Finish patch would restore features, languages, proficiencies, attacks, inventory, resources, HP, AC, and calculation metadata the player deleted. **`underCapAckLevels` is neither read, written, cleared, nor used as a gate** — this flow reaches no new level and records no decision to finish short. `rest.preparedByClass`, compatibility `preparedIds`, stored Long Rest state, `characters.snapshots`, the required-choice banner, Complete Choices, manual rows/notes, and unrelated build keys are all byte-identical. **Derived** prepared capacity is deliberately *not* claimed to be byte-identical: `effectiveCapacity` is bounded by the candidate set, and a wizard prepares from its spellbook, so adding a spellbook entry legitimately raises it — a derivation from the new candidate, never a write to prepared state. **No schema change, migration, new persisted field, load-time repair, or normalization.** The Combat embedded Spells panel is unmodified — it keeps its direct `+ Level` button, exposes no correction, and still displays the new canonical rows through the one shared `initSpellsPanel()`. Tests: new `tests/spellbookChoices.test.js` (39), `tests/spellsPanelOverflowMenu.test.js` (23), `tests/spellbookChoicesWiring.test.js` (3), `tests/smoke/spellbookChoices.smoke.js` (2), plus 11 new cases in `tests/builderSheetSeeding.test.js` (29 → 40). Every negative case carries a positive control; the new suites were run against reverted production code first, and the class-specific ceiling was additionally proved by a mutation control (a non-class-specific ceiling makes the Wizard 1 / Cleric 19 case report 9 instead of 1). Verified: typecheck clean, 1708/1708 unit, build clean, both smoke gates 83/83, plus a real 380 × 820 production-preview pass. **Not in this batch:** max-level cantrip/known-spell recovery (step 33), R5-C, R6, snapshot deletion. **Independent-review correction (same day, second commit):** the first commit's sheet patch called the unfiltered whole-build spell seeder, so an addition also resurrected builder rows the player had deleted, filled empty slot totals, and canonically reordered levels — `getSpellbookAdditionSheetPatch(character, additions, registry)` now takes the explicit chosen ids and seeds only those, leaving every pre-existing level and row byte-identical. Multi-class Apply became atomic (all buckets validated before the first assignment; a missing bucket is created in the same transaction rather than rejected after a partial write). A successful commit now emits the established `"spells"` panel invalidation so an already-mounted Combat Spells panel updates live. `#spellbookChoicesPanel` gained narrowly scoped copies of the six spell-list/checkbox/count/disabled/validation rules it renders — the originals are scoped to the two wizard panels, so the dialog had been shipping unstyled. The overflow menu gained `menu`/`menuitem` semantics with `aria-controls`/`aria-labelledby`, focus restoration to the trigger after Add spell level completes or is cancelled, and a no-Popovers fallback that honors Escape, outside click, and teardown. **Independent-review correction (same day, third commit):** the second commit's Apply still classified buckets with `isPlainObject(x) ? x : null`, so a **present but malformed** `build.spellcasting` root or class bucket (an array, a string, a number, `null`) was indistinguishable from a genuinely absent one and was silently created over, destroying the stored value. The new `planSpellbookAppends()` resolves every addition before the first live assignment and answers the two cases differently: a genuinely **absent** root or class bucket is a legal state that eligibility already counts as `0 chosen`, so it is created inside the same transaction, while a **present malformed** root or class bucket — or a present bucket whose `knownIds` is neither absent nor an array — rejects the **entire** Apply before any write, so every class survives byte-for-byte with no dirty mark, no rerender, no panel invalidation, and no success status (the failure is announced through the existing alert and the dialog stays open). Four regressions took `tests/spellbookChoices.test.js` 35 → 39: a malformed top-level root; a valid first multiclass bucket followed by a malformed second class bucket, proving the first class's append does not survive the second's refusal; a positive control for a genuinely absent root; and a surgical negative control mirroring the pre-fix classification. Both rejection cases were confirmed to fail against the pre-fix module and pass against this one.
33. [ ] **Max-level cantrip / known-spell correction — MANDATORY GATE before R5-C; requires its own design and explicit owner authorization.** The level-20 seam closed for the spellbook in step 32 is **identical** for the other two permitted under-cap categories: a character at `MAX_CHARACTER_LEVEL` carrying an unsatisfied `UNDER_CAP_KIND.CANTRIPS` or `UNDER_CAP_KIND.KNOWN_SPELLS` descriptor has no Level Up left to fill it and, once Edit in Builder retires, no route at all. Step 32 deliberately did **not** generalize to them (owner ruling: spellbook only). This correction must be separately designed, placement-confirmed, and authorized **before** R5-C removes Edit in Builder. Do not self-start it, and do not fold it into R5-C.
34. [ ] **Restore Character phases R5-C, R6 (spec §8) + snapshot deletion: each still requires explicit owner authorization before code changes.** The binding sequence is **step 32 (spellbook correction, shipped) → step 33 (max-level cantrip/known-spell correction) → R5-C Edit-in-Builder retirement (bound by the D1–D4 rulings) → R6 shipped-behavior doc close-out.** R5-C must not begin until step 33 has shipped. **Snapshot deletion remains a separately authorized future phase — NOT part of R3/R4/R5/R6 (spec §5).** Do not self-start any phase from this list.
35. [ ] **Still open after F2 (needs new owner scope):** the remaining P2 backlog in `docs/audits/builder-completion-matrix.md` §3 (feature-action counters, partial-regain recovery, prepared-formula overrides, equipment depth, keyboard a11y pass), the deferred choice work above (Half-Elf ability/skill choices, Tiefling cantrip grant, race-trait fixed proficiencies, Dwarf tools), and freeform initiative (audit F1). (Preview-compatibility for the dev-only smoke harnesses was completed 2026-07-18 — see step 17.)
36. [ ] **Still blocked / out of scope:** down-leveling (ratified out of scope; the undo path is Restore Character per step 18), builtin content expansion beyond the SRD 5.1 greenlist, and any work outside the character-builder system
