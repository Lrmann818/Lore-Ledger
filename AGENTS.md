...
17. [ ] **Audit F2 — AC / max HP / spell save DC / spell attack snapshot→derived: authorized 2026-07-15 (owner), in progress.** Bring the four remaining snapshot fields under the calculation contract (derived by default, explicit adjustment, intentional fixed override) using an optional per-field `calc` block on the open character-entry shape (the shipped attack-`calc` / `builderSeed` precedent — no schema migration), with a no-calc-block legacy default that preserves every existing sheet. Batch order: (1) docs — Phase A audit + contract "Structured Vitals" model + this authorization; (2) spell save DC + spell attack (per-source profiles, freeform caster profiles); (3) Armor Class; (4) max HP (builder-derived; freeform stays manual); (5) integration + docs. Scope is strictly these four fields. See `docs/audits/character-calculation-audit-2026-07.md` → "Phase A — F2 field-by-field audit" and `docs/reference/character-calculation-contract.md` → "Structured Vitals ownership".
18. [ ] **Still open after F2 (needs new owner scope):** the remaining P2 backlog in `docs/audits/builder-completion-matrix.md` §3 (feature-action counters, partial-regain recovery, prepared-formula overrides, equipment depth, keyboard a11y pass) and the deferred choice work above (Half-Elf ability/skill choices, Tiefling cantrip grant, race-trait fixed proficiencies, Dwarf tools)
