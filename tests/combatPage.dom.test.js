...
describe("combat page style contracts", () => {
  ...
  it("keeps combat HP and AC markup on the compact centered stat-tile classes", () => {
    ...
    // AC is an editable input styled as a stat tile (merged from builder-wizard):
    // typing writes back to the linked character/tracker card via setCombatParticipantAc.
    expect(combatPageSource).toContain('acField.className = "combatAcField combatStatTile combatVitalTile"');
    expect(combatPageSource).toContain('acLabel.className = "combatAcLabel combatStatLabel"');
    expect(combatPageSource).toContain('acInput.className = "combatAcInput combatStatValue"');
    expect(combatPageSource).toContain('acInput.dataset.combatAcInput = "true"');
    expect(combatPageSource).toContain('acInput.setAttribute("aria-label", `Armor Class for ${card.name}`)');
    expect(combatPageSource).toContain("setCombatParticipantAc(state, participantId, getAcInputValue(target))");
