...
- [`reference/restore-character-spec.md`](./reference/restore-character-spec.md) — **Normative, ratified 2026-07-18. Phase R1 (schema v13 + transactional pre-Level-Up snapshot capture) shipped 2026-07-18; phase R2 (non-UI restore engine) shipped 2026-07-22; phase R3 (user-facing, restore-only Restore Character UI) shipped 2026-07-22; phase R4 (backup asset completeness) shipped 2026-07-24; phases R5-A, R5-B1, and R5-B2 shipped 2026-07-24 / 2026-07-25 / 2026-08-02; R5-C and R6 still require owner authorization, and snapshot deletion is deferred to a separately authorized future phase (not R3/R4/R5/R6).** Pre-Level-Up character snapshots (campaign-scoped `characters.snapshots`, one-vault-write transaction with the Level Up commit) and the Restore Character flow that restores any snapshot as a separate playable copy. Defines the data model, identity/naming rules, backup behavior, dialog UX (incl. the persistence-confirmation / `Retry Save` contract), and implementation phases R1–R6.
- 📸 **Session snapshot** — [`reference/session-handoff-2026-07-22-r2-restore-engine.md`](./reference/session-handoff-2026-07-22-r2-restore-engine.md) — handoff for the 2026-07-22 R2 run: the non-UI restore engine (snapshot resolution, migrate-through preparation, identity/naming, spell-row regeneration + note copies, staged commit with rollback), 40 new tests, both smoke gates, the temporary production-preview inspection harness, and the R3 authorization gate.
