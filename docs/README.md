...
- [`reference/restore-character-spec.md`](./reference/restore-character-spec.md) — **Normative, ratified 2026-07-18. Phase R1 (schema v13 + transactional pre-Level-Up snapshot capture) shipped 2026-07-18; phase R2 (non-UI restore engine) shipped 2026-07-22; R3–R6 still require owner authorization.** Pre-Level-Up character snapshots (campaign-scoped `characters.snapshots`, one-vault-write transaction with the Level Up commit) and the Restore Character flow that restores any snapshot as a separate playable copy. Defines the data model, identity/naming rules, backup behavior, dialog UX, and implementation phases R1–R6. No user-facing Restore UI exists yet.
