...
- [`reference/restore-character-spec.md`](./reference/restore-character-spec.md) — **Normative, ratified 2026-07-18; implementation not yet authorized.** Pre-Level-Up character snapshots (campaign-scoped `characters.snapshots`, schema v13, one-vault-write transaction with the Level Up commit) and the Restore Character flow that restores any snapshot as a separate playable copy. Defines the data model, identity/naming rules, backup behavior, dialog UX, and implementation phases R1–R6.
