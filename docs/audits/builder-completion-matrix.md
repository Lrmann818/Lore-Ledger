...
| 15 | Custom content authoring UX | 🟡 | `js/ui/customContentManager.js`, `js/domain/customContentAuthoring.js` | n/a | **Shipped 2026-07-13 (batches 1–4):** Manage Custom Content dialog lists/removes every record (removal confirms with the names of referencing characters) and creates/edits **custom spells**, **custom feats** (prerequisites + the closed `effects` vocabulary), and **custom races** (size/speed/ASIs/languages/lore plus inline **trait sub-records** saved as separate `kind:"trait"` records — all-or-nothing multi-record saves, orphaned traits cleaned up on edit) through full forms (inline plain-language validation, generated ids locked on edit, same domain rules as JSON import, 380px verified) | No guided authoring yet for classes (incl. `resources[]`/`grantedSpells`); race build-time `choices[]` and subraces not authorable (imported ones preserved verbatim) | P1 | #14 | 2 | A user can author a working custom class without hand-writing JSON |
