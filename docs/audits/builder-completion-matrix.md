...
| 15 | Custom content authoring UX | 🟡 | `js/ui/customContentManager.js`, `js/domain/customContentAuthoring.js` | n/a | **Shipped 2026-07-13 (batches 1–3):** Manage Custom Content dialog lists/removes every record (removal confirms with the names of referencing characters) and creates/edits **custom spells** and **custom feats** (prerequisites + the closed `effects` vocabulary as repeatable rows) through full forms (inline plain-language validation, generated ids locked on edit, same domain rules as JSON import, 380px verified) | No guided authoring yet for races (incl. trait sub-records) or classes (incl. `resources[]`/`grantedSpells`) | P1 | #14 | 2 | A user can author a working custom class without hand-writing JSON |
