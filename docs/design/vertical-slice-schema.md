...
**Implementation status (as of April 30, 2026):** Steps 1–5 are complete through Phase 3B, and the Phase 3C, Phase 3D, Phase 3E, Phase 3F, Phase 3G, Phase 3H, Phase 3I, and Phase 3J foundation/status slices are complete. Phase 3B derives the selected Dragonborn ancestry mechanics from the stored ancestry ID plus registry/rules data and displays those mechanics in Builder Summary. The current temporary builder-specific surface shows damage resistance, breath weapon damage type/area, save ability, save DC, and damage scaling as live review scaffolding, while normal panels own the final table/play surfaces. Breath Weapon DC also appears in Vitals when derivable because it is a table-use combat DC. Phase 3C foundation complete: derived Dragonborn Breath Weapon now renders as the first display-only Abilities & Features card, with no mutation into Weapons, Spells, Equipment, or flat character fields. Phase 3D foundation complete: Character page Short Rest / Long Rest toolbar controls now route through a central active-character recovery helper for explicitly tagged `character.resources[]` counters. Phase 3E foundation complete: Vitals resource recovery metadata can now be configured from resource tiles through press-and-hold or keyboard activation, without visible tile settings buttons, and the dialog writes only `resource.recovery`. Phase 3F foundation and first polish pass complete: freeform characters and builder characters can use the same normal Abilities & Features panel for user-created manual/custom cards while builder-derived cards remain derived/read-only and separate from manual persisted state. Manual cards can be added, edited, deleted, reordered, collapsed, and given independently collapsible notes/descriptions; management actions live behind a gear/settings menu. Manual cards support optional plain-text attack, damage, and effect fields while preserving legacy `damageEffect`, but no attack/damage calculation or resource automation is implied. Phase 3G foundation complete: manual/custom cards can optionally persist a feature-specific `limitedUse` counter with a label, current/max uses, and recovery metadata; enabled cards render compact use/restore-one/reset controls, and eligible counters recover through the existing character-level rest helper. Phase 3H foundation complete: derived Dragonborn Breath Weapon now renders use/restore-one/reset controls backed only by `character.featureUses["dragonborn-breath-weapon"].current`, defaults missing use state to full uses, and recovers on Short Rest or Long Rest through the same character-level rest helper. Phase 3I foundation complete: Dragonborn builder wizard Finish seeds the selected Draconic Ancestry and Damage Resistance into the existing editable Features / Traits field, and fixed Common/Draconic languages into the existing Languages field, while preserving existing text and keeping Breath Weapon live-derived. Phase 3J status cleanup complete: Builder Summary remains visible as temporary review/comparison scaffolding, Builder Identity and Builder Abilities remain active, stale copy was clarified, and no schema/data/migration/seeding behavior changed. Partial regain behavior, generalized seeding, level-up additions, broad shared-resource-linked cards, spell slot recovery, combat/linked-character rest behavior validation, broader class-feature automation, broad derived feature-use automation, fuller menu keyboard accessibility, and possible `damageEffect` / `effectText` cleanup remain future work.

The derived resources, derived combat stats, and feature-action pattern is now: table-use values should have a home in the normal character sheet panel where users need them, not only in Builder Summary. Vitals owns compact derived stats and broad shared canonical resource counters. Weapons owns normal weapon/equipment attacks. Spells owns actual spells. Abilities & Features owns special feature/action mechanics such as Breath Weapon. Feature-specific limited-use counters, such as Second Wind uses, Relentless Endurance uses, or Vampiric Bite's empowered-use counter, can surface on manual/custom feature cards through the Phase 3G `limitedUse` foundation. Derived Dragonborn Breath Weapon uses now surface on the derived card through Phase 3H `featureUses`, while max uses, recovery, label, DC, area, damage, damage type, ancestry, and feature text remain derived from build/rules data. Derived Breath Weapon is not copied into `manualFeatureCards[]` and is not stored as `character.resources[]`. Feature cards may reference, spend, restore, or explain broad shared resources such as Sorcery Points, Ki, Bardic Inspiration, Rage uses, or Channel Divinity, but they must not duplicate those shared counters. Spell slots, Pact Magic slots, and prepared/known spell automation remain future spell-system work. Rest buttons belong in character-level toolbar actions, not inside the panel. Phase 3D defines the recovery vocabulary as `shortRest`, `longRest`, `shortOrLongRest`, `manual`, and `none`; the current rest helper changes active-character `character.resources[]` counters, Phase 3G manual/custom `manualFeatureCards[].limitedUse` counters, and Phase 3H Dragonborn Breath Weapon `featureUses` with explicit matching recovery metadata, while intentionally leaving untagged/manual existing resources and unrelated/stale feature counters unchanged. Phase 3H does not include Sorcery Points, Ki, Bardic Inspiration, Rage uses, Channel Divinity, Metamagic, Flexible Casting, spell slots, Pact Magic, attack/damage calculation, AC derivation, equipment automation, generated SRD expansion, or broad class-feature automation. Phase 3E assigns recovery metadata to existing Vitals resource trackers without adding visible settings buttons, gears, or ellipses to the compact tiles: pointer/touch users open settings with press-and-hold on the resource tile body, keyboard users open settings with Enter or Space on a focused tile, and long-press ignores gestures that start on tile controls such as current/max inputs, increment/decrement buttons, or delete buttons. The Vitals tip explains the press-and-hold path, missing recovery displays as Manual, Save preserves existing resource values while writing only `resource.recovery`, and Cancel/Escape close without mutation. Builder-only panels are temporary scaffolding; before any such panel is retired, every useful value it shows needs a normal-panel home. Builder characters should populate normal panels through derivation, seeded editable content, and intentional overrides, while freeform characters can create manual feature cards or remain blank/minimal until edited. Builder-derived and freeform/manual cards render through the same Abilities & Features panel UI, without duplicating builder-derived cards into manual/freeform card state unless a later explicit copy, customize, or override behavior is designed. Combat participant cards now show compact HP and manually editable AC beside HP; AC follows the existing linked-card source pattern when a linked character or tracker card exists. This is not AC derivation, armor/equipment automation, builder AC automation, or a broader character AC system.

Phase 3I clarified the editability model behind that surface routing. Builder-created characters are intended to stay editable after creation for homebrew, house rules, campaign rulings, and manual corrections. Registry/rules data may seed editable content into normal sheet fields or entries during wizard creation, including Features / Traits, Proficiencies & Languages, Equipment, Spells, Weapons, Vitals resources, or Abilities & Features when those are the correct homes. Once content is written into an editable sheet field or entry, that persisted content is user-owned. Later builder/rules additions, especially during level-up, may append or offer duplicate-aware updates, but they must preserve existing edits and should be explicit where practical. This does not mean every rules-derived value becomes editable content: compact calculations and counters such as proficiency bonus, derived DCs, level-based dice, and feature-use maximums can remain live-derived. Builder-created feature cards need a required future edit/reorder/customization path, but cards that mix derived mechanics with user content need an explicit persisted customization model so user-edited content is not silently overwritten.

The implemented Phase 3I slice is intentionally narrower than the full model: it seeds only Dragonborn passive/descriptive trait text and fixed race languages at initial wizard Finish. It does not add level-up append flows, generalized race/class/background seeding, language choice handling, new schema fields, or builder-created card customization.
