...
**Implementation status (as of April 29, 2026):** Steps 1–5 are complete through Phase 3B, and the Phase 3C and Phase 3D foundation slices are complete. Phase 3B derives the selected Dragonborn ancestry mechanics from the stored ancestry ID plus registry/rules data and displays those mechanics in Builder Summary. The current temporary builder-specific surface shows damage resistance, breath weapon damage type, breath weapon area/shape, save ability, save DC, and damage scaling. Breath Weapon DC also appears in Vitals when derivable because it is a table-use combat DC. Phase 3C foundation complete: derived Dragonborn Breath Weapon now renders as the first display-only Abilities & Features card, with no use tracking or mutation into Weapons, Spells, Equipment, or flat character fields. Phase 3D foundation complete: Character page Short Rest / Long Rest toolbar controls now route through a central active-character recovery helper for explicitly tagged `character.resources[]` counters. Manual recovery metadata UI, feature-use tracking, Breath Weapon use tracking, spell slot recovery, and broader class-feature automation remain future work. Visual/card polish, manual/freeform feature cards, user-created/custom feature cards, use tracking beyond explicit resource counters, and broader feature coverage also remain future work.

The derived resources, derived combat stats, and feature-action pattern is now: table-use values should have a home in the normal character sheet panel where users need them, not only in Builder Summary. Vitals owns compact derived stats and canonical resource counters. Weapons owns normal weapon/equipment attacks. Spells owns actual spells. Abilities & Features owns special feature/action mechanics such as Breath Weapon. Feature cards may reference or spend a resource, but the resource itself must have one canonical counter. Rest buttons belong in character-level toolbar actions, not inside the panel. Phase 3D defines the recovery vocabulary as `shortRest`, `longRest`, `shortOrLongRest`, `manual`, and `none`; the current foundation changes only active-character `character.resources[]` counters with explicit matching recovery metadata and intentionally leaves untagged/manual existing resources unchanged. Phase 3E is planned as a Resource Recovery Settings Dialog for assigning that metadata to existing Vitals resource trackers without adding visible settings buttons, gears, or ellipses to the compact tiles. Pointer/touch users should open it with press-and-hold on the resource tile, keyboard users should open it with Enter or Space on a focused tile, and long-press must ignore gestures that start on tile controls such as current/max inputs, increment/decrement buttons, or delete buttons. Builder-only panels are temporary scaffolding; before any such panel is retired, every useful value it shows needs a normal-panel home. Builder characters should populate normal panels through derivation and intentional overrides, while freeform characters should eventually create manual feature cards. Builder-derived and freeform/manual cards should render through the same Abilities & Features panel UI, without duplicating builder-derived cards into manual/freeform card state unless a later explicit copy, customize, or override behavior is designed.
