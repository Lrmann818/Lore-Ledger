...
The native iOS / App Store / TestFlight procedure lives in
[`ios-packaging.md`](./ios-packaging.md).

It is a separate document because it is a separate release path: the web release described
in this document does not touch Capacitor, Xcode, or App Store Connect. Run the web build,
versioning, and smoke-test steps above first; the iOS doc picks up from the built `dist/`.
