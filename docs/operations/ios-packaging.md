# App Store / iOS Packaging (Capacitor)

*Extracted from `release-process.md` on 2026-07-09. That doc owns the **web** release
procedure; this one owns the **native iOS / App Store** path.*

Read with [`release-process.md`](./release-process.md) — the web build, versioning, and
smoke-test gates there run **before** anything here.
...
> ⚠️ **SUPERSEDED — point-in-time audit. Do not rely on its determination.**
>
> This audit was run on 2026-05-16, **before the SRD 5.1 character builder shipped**. At
> that time the app contained no SRD-derived content, so it concluded no attribution was
> required. That is **no longer true**: the app now ships `game-data/srd/*.json` (races,
> classes, subclasses, Acolyte, Grappler, 319 spells, equipment, packs).
>
> Attribution posture is owned by
> [`../reference/srd-licensing-notes.md`](../reference/srd-licensing-notes.md) and
> [`../reference/attribution-requirements.md`](../reference/attribution-requirements.md).
> **Read those before any App Store submission.** The table below is retained only as a
> record of what the pre-builder build contained. It also cites `NEW-FEATURES-ROADMAP.md`
> and `MULTI-CHARACTER_DESIGN.md`, which no longer exist under those names.

**Determination (2026-05-16, now superseded): SRD attribution is NOT required for this version.**
