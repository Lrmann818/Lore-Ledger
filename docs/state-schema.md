...
- Step 3 Phase 3B also did not change the schema. The Builder Identity panel edits only `build.raceId`, `build.classId`, `build.backgroundId`, and `build.level` for builder characters, using builtin SRD-safe content IDs from the code-shipped registry. Selecting "Not selected" stores `null` for the relevant content ID. It does not persist derived values into flat fields, and it does not add ability editing, subclass choices, custom content, HP/AC/spell automation, or a full builder wizard.
