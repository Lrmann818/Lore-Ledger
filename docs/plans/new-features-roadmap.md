...
- [ ] **Restore Character implementation (phases R1–R6, each needs explicit owner authorization)** — R1 schema v13 + Level Up capture → R2 restore engine → R3 Restore Character UI → R4 backup asset completeness → R5 Edit-in-Builder retirement (gated on owner decisions D1–D3) → R6 shipped-behavior doc close-out. See `docs/reference/restore-character-spec.md` §8 and the AGENTS.md working order
