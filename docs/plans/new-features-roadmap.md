...
- [x] **Attack Recalculate from Build (matrix #9)** — shipped 2026-07-14 (owner-authorized after a green #15 integration checkpoint): stable `weapon:<id>` provenance on seeded attacks, explicit per-row action with a field-by-field preview and per-field acceptance, name/notes always user-owned, explicit weapon linking for legacy rows, atomic apply, no automatic rewrites ever
