...
- [x] **Custom content authoring UX (matrix #15)** — complete 2026-07-14 (batches 1–5): Manage Custom Content dialog authors spells, feats, races (inline trait sub-records), and classes (inline feature sub-records, standard-SRD-table spellcasting, `resources[]`, `grantedSpells`) through forms with inline validation, locked ids, and all-or-nothing multi-record saves; the same session fixed the campaign-vault bug that dropped `state.content` on reload. Not form-authorable (JSON import, preserved verbatim): subclasses/subraces, `choices[]`, multiclassing, starting equipment, threshold recovery, custom slot tables
- [x] **Authorized builder-completion batches (2026-07-12 → 2026-07-14) — complete.** Every P1 in `docs/audits/builder-completion-matrix.md` is closed; the remaining open items are all P2 and need new owner scope (matrix §3)
- [x] **Attack Recalculate from Build (matrix #9)** — shipped 2026-07-14, then **superseded 2026-07-15** by the unified calculation contract (below)
- [x] **Unified character calculation contract + live-deriving attacks** — shipped 2026-07-15 (owner-authorized): Builder and manually entered characters share one rules engine (`docs/reference/character-calculation-contract.md`); attacks derive live through one canonical calculator (`js/domain/attackCalculation.js`) with a structured `calc` block, explicit adjustments, intentional fixed mode, marker-based provenance/re-seed dedup, and safe legacy conversion in a per-row editor. The broken "Recalculate from Build" dialog was removed. AC/HP/spellDC/spellAttack remain snapshot fields (audit F2, deferred)
- [x] **Reusable choice-based granted spells + High Elf wizard cantrip** — shipped 2026-07-15: filtered spell choices (`{ source: "spells", filter: { classId, maxLevel } }` + `spellcastingAbility`) render in the wizard, gate Finish, seed for casters and non-casters, and carry Intelligence provenance; generic mechanism in `js/domain/rules/spellChoices.js`
- [x] **Structured Vitals (audit F2): live-deriving spell DC/attack, AC, and max HP** — shipped 2026-07-16/17 (owner-authorized 2026-07-15): optional per-field calc blocks on the open entry shape (`spellcastingCalc` per-source profiles, `acCalc`, `hpMaxCalc`; no migration) — derived by default for new builder characters with live updates, explicit adjustments, intentional fixed overrides, and verbatim legacy snapshots with editor-based adoption (adoption-safety stamp rule). Engine gains the Defense fighting style (+1 while armored) and structured Dwarven Toughness (`hpPerLevelBonus`). Calc-aware Finish/Level Up policies, rest healing to the displayed max, tracker/combat surfaces resolving one displayed value, temporary combat AC layered over the calculated base. Freeform casters declare DC/attack profiles; freeform AC/max HP stay manual by contract. New production-preview smoke gate (`playwright.preview.config.js`). See `docs/reference/character-calculation-contract.md` → "Structured Vitals ownership" and the calculation audit → Phase B
- [x] **Restore Character / pre-Level-Up snapshot specification** — ratified and specified 2026-07-18 (owner-directed, specification only): before every successful Level Up commit, Lore Ledger saves a complete character snapshot in the same vault write; a Restore Character dialog restores any snapshot as a separate playable copy (new stable ID, never overwrites the current or source character, repeatable); snapshots live in campaign-scoped `characters.snapshots` (schema v13), ride through vault/backup round trips, and are retained until individually deleted. Includes the ratified retirement of general user-facing Edit in Builder. Normative spec: `docs/reference/restore-character-spec.md`; dependency audit + open owner decisions D1–D4: `docs/audits/edit-in-builder-retirement-audit-2026-07.md`
- [x] **Restore Character R1 — schema v13 + transactional pre-Level-Up snapshot capture** — shipped 2026-07-18 (owner-authorized): campaign-scoped `characters.snapshots`, one complete deep-cloned snapshot captured inside the same state mutation as every successful Level Up commit (never on open/cancel/invalid apply; `(kind, sourceCharacterId, fromLevel)` replace-append dedup), records round-trip through sanitize/vault/full backups and survive source-character deletion; owner decisions D1–D4 ruled the same day. No user-facing Restore UI yet — history is created and preserved only
- [x] **Restore Character R2 — non-UI restore engine** — shipped 2026-07-22 (owner-authorized): `js/domain/characterSnapshots.js` resolves a snapshot, prepares a separate playable copy through the canonical migration pipeline (new stable id, regenerated spell-row ids with note-copy mapping, all other character-local ids preserved, provenance fields, deterministic `— Restored Level <N>` naming with collision suffixes), and commits it with staged portrait/note copies, rollback on failure, a double-commit guard, and no default activation. `restoreCharacterFromSnapshot` is the seam the R3 UI will call. 40 focused tests; both smoke gates green. No user-facing restore exists yet — the engine has no production caller until R3
- [x] **Restore Character R3 — user-facing, restore-only Restore Character UI** — shipped 2026-07-22 (owner-authorized): `Restore Character` in the character action menu (directly after Level Up, always available) opens a grouped, newest-first snapshot dialog that confirms non-destructively and restores a snapshot as a separate playable copy through the R2 `restoreCharacterFromSnapshot` engine (`activate: true`) — no restoration logic duplicated in the UI. Owner-ratified persistence-confirmation contract: after the engine commits in memory, R3 confirms the save via `SaveManager.flush()`; an unconfirmed save locks the dialog open with `Retry Save` (retries flush only, never the engine) and finalizes only on a confirmed save (notify → rerender → `Restored "<name>"`). Restore-only — no snapshot deletion UI. `js/pages/character/restoreCharacterDialog.js`, `#restoreCharacterOverlay`, `characterPage.js` wiring, `migrateState`/`deleteText` threaded via `trackerPage.js`/`app.js`; 27 focused unit tests + a new smoke suite; both smoke gates 64/64
- [ ] **Restore Character implementation (remaining phases R4–R6, each needs explicit owner authorization)** — R4 backup asset completeness (snapshot-payload blob/text collectors) → R5 Edit-in-Builder retirement (design bound by the D1–D4 rulings) → R6 shipped-behavior doc close-out. **Snapshot deletion is a separately authorized future phase — not R3, and not R4/R5/R6 (spec §5).** See `docs/reference/restore-character-spec.md` §8 and the AGENTS.md working order
- [ ] **Deferred choice work (documented, needs owner scope)** — Half-Elf ability (+1×2) and Skill Versatility choices, Tiefling Infernal Legacy cantrip grant, race-trait fixed proficiencies (Keen Senses, Elf Weapon Training, …), Dwarf tool choice. See `docs/audits/character-calculation-audit-2026-07.md`
- [ ] **Still blocked / out of scope —** down-leveling (the ratified undo path is Restore Character, above) and builtin content expansion beyond the SRD 5.1 greenlist
