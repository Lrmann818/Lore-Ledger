...
- [ ] **Restore Character implementation (remaining phases R3–R6, each needs explicit owner authorization)** — R3 Restore Character UI → R4 backup asset completeness (snapshot-payload blob/text collectors) → R5 Edit-in-Builder retirement (design bound by the D1–D4 rulings) → R6 shipped-behavior doc close-out. See `docs/reference/restore-character-spec.md` §8 and the AGENTS.md working order
